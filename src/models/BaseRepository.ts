...
export function convertTimestampToDate(data: Record<string, unknown>): Record<string, unknown> {
  return Object.fromEntries(
    Object.entries(data).map(([key, val]) => {
      if (val instanceof firebase.firestore.Timestamp) return [key, val.toDate()]
      else if (Array.isArray(val))
        return [
          key,
          val.map((v) =>
            v instanceof firebase.firestore.Timestamp ? v.toDate() : isObject(v) ? convertTimestampToDate(v) : v
          ),
        ]
